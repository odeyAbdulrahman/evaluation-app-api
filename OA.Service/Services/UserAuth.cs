...
namespace OA.Service.Services
{
    class UserAuth : IUserAuth
    {
        ...
        public bool AnyByEmailAndAccountStatus(string email, bool accountStatus)
        {
            Email = email; AccountStatus = accountStatus;
            return UserManager.Users.Any(x => x.Email != Email && x.AvailabilityStatus == AccountStatus);
        }
        ...
        public async Task<string> GeneratePasswordResetTokenAsync(AspNetUser model)
        {
            return await UserManager.GeneratePasswordResetTokenAsync(model);
        }
        public async Task<IdentityResult> ResetPasswordAsync(AspNetUser model, string code, string password)
        {
            return await  UserManager.ResetPasswordAsync(model, code, password);
        }
    }
}
